...
function register_variable($variable_name=null){
	if ($variable_name) {
...
function get_ad_info($ad_id,$db){
	...
	$db->where('a.id',$ad_id);
	$info = $db->getOne('ad_reference a', 'a.id, a.title, source_id, large_width, large_height, tap_url, code, date_modified');
	return $info;
}
...
function delete_comic_images($comic_page_id,$db){
	...
	if ( $image_reference_id_set ) {
		$sql = 'DELETE FROM grlx_image_reference WHERE id IN ('.$image_reference_id_set.') LIMIT '.count($image_reference_id_list);
...
SELECT
	...
	large_image_url
FROM
	grlx_ad_slot_match am,
	grlx_ad_reference ar
WHERE
	am.slot_id = ?
...
/*
function fetch_book_pages($book_id='',$db){
	if ( $book_id ) {
		$sql = "
SELECT
	cc.id AS chapter_id,
